...
module PGExtra::Operations::Tables
  # @private
  #
  ...
  #   and checking them against the content of `pg_extra`;
  ...
  #
  class CreateTable < PGExtra::Operation
    ...
    # We rely on the fact all tables of interest are registered in `pg_extra`.
    from_sql do
      <<~SQL
        SELECT
          c.oid,
          (c.relnamespace::regnamespace || '.' || c.relname) AS name
        FROM pg_class c JOIN pg_extra p ON p.oid = c.oid
    ...
    #
    # @see +PGExtra::SchemaDumper+ module (in `core/railtie`).
    def to_ruby
      stream = StringIO.new
      PGExtra.dumper.send(:table, name.lean, stream)
    ...
    # we do indentation separately in the +PGExtra::SchemaDumper+.
    #
