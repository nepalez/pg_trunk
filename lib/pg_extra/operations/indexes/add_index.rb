...
module PGExtra::Operations::Indexes
  # @private
  #
  # PGExtra excludes indexes from table definitions provided by Rails.
  ...
  #
  class AddIndex < PGExtra::Operation
    ...
    from_sql do
      <<~SQL
        ...
        FROM pg_class c
          ...
          JOIN pg_extra p ON p.oid = c.oid
    ...
    def to_ruby
      indexes = PGExtra.database.send(:indexes, table.lean)
      index = indexes.find { |i| i.name == name.lean }
      return unless index

      line = PGExtra.dumper.send(:index_parts, index).join(", ")
